...
from unsupervised.encoder import ZINCEncoder
...
def run(args):
    ...
    if args.full:
        ...
        logging.info('Using Subset of ZINC 10K/1K/1K')

    setup_seed(args.seed)

    evaluator = ZINCEvaluator()
    dataset = ZINC("./original_datasets/ZINC", subset=True, split='all', transform=my_transforms)

    train_dataset = ZINC("./original_datasets/ZINC", subset=True, split='train')
    val_dataset = ZINC("./original_datasets/ZINC", subset=True, split='val')
    test_dataset = ZINC("./original_datasets/ZINC", subset=True, split='test')
    ...
    model = GInfoMinMax(
        ZINCEncoder(dataset.num_atom_type, dataset.num_bond_type, emb_dim=args.emb_dim, num_gc_layers=args.num_gc_layers,
                    drop_ratio=args.drop_ratio, pooling_type=args.pooling_type),
        args.emb_dim).to(device)

    model_optimizer = torch.optim.Adam(model.parameters(), lr=args.model_lr)
    view_learner = ViewLearner(
        ZINCEncoder(dataset.num_atom_type, dataset.num_bond_type, emb_dim=args.emb_dim,
...
def arg_parse():
    ...
    parser.add_argument("--full", default=False, action="store_true",
                        help="Flag to use full zinc dataset")
