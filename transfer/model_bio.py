...
class GINConv(MessagePassing):
	...
	def forward(self, x, edge_index, edge_attr, edge_weight= None):
		...
		if edge_weight is not None:
			self_loop_weights = torch.ones(self_loop_attr.shape[0], dtype=torch.float).to(edge_weight.device).to(
				edge_weight.dtype)
	...
	def message(self, x_j, edge_attr, edge_weight):
		...
		return torch.cat([x_j, edge_attr], dim=1) if edge_weight is None else torch.cat([x_j, edge_attr], dim=1) * edge_weight.view(-1, 1)
...
class BioGNN_graphpred(torch.nn.Module):
	"""
	Extension of GIN to incorporate edge information by concatenation.
	...
	def __init__(self, num_layer, emb_dim, num_tasks, JK="last", drop_ratio=0, graph_pooling="mean", gnn_type="gin"):
		super(BioGNN_graphpred, self).__init__()
		self.num_layer = num_layer
		self.drop_ratio = drop_ratio
		self.JK = JK
		self.emb_dim = emb_dim
		...
		self.gnn = BioGNN(num_layer, emb_dim, JK, drop_ratio, gnn_type=gnn_type)
